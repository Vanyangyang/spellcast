...
**对话之外的舞台。** 用桌面旁念和画布，与 Codex 一起发展想法。
...
这段早期演示里，我正在用 Codex CLI 开发一个日历应用。Spellcast 把一些我没想到的点子以桌面气泡弹出来；我给其中一个点星、拖动，再带到画布上头脑风暴、继续发展。真实 Windows 桌面、真实鼠标操作，只剪掉了等待，没有任何合成。当前的直接反馈流程使用 Codex Desktop。
...
- **当前源码和已在本机测试的 Windows 运行版：** 可编辑画布组件、带名称的 Idea 组合、按工作区和任务整理、直接回发原 Codex Desktop 任务、一键完整接入、按发送请求展示的回复面板。这些更新尚未包含在公开的 0.3.0 安装包中。详见[运行验收](docs/reviews/2026-09-14-workbench-runtime-acceptance.md)、[回复面板](docs/reviews/2026-09-15-replies-inbox.md)和[仅 Codex 接入](docs/reviews/2026-09-15-codex-only-entry.md)。
...
明确发送后，Spellcast 通过运行中的 **Codex Desktop** 直接把请求交给对应任务，包括当前空闲的任务；结果可以回写原画布内容。应用关闭、任务删除、工具不可用或投递失败时，不会把请求显示为已成功执行。完整接入包含 MCP、Hooks 和行为说明，收到请求与实际完成分别记录。
...
1. 按下文从当前源码运行，使用这里描述的新流程。[公开的 Windows 体验版](https://github.com/Vanyangyang/spellcast/releases/latest)仍是较早的快照。
2. 打开**设置**，为 Codex 点击**安装 / 更新 Spellcast 接入**，一次安装 MCP、Hooks 和行为 Skill，并完成备份及冲突检查。
3. 重载 Codex，按接入状态中的提示信任 Hooks。需要独立旁念时，在 Spellcast 中打开旁念开关。

接入详情：[docs/codex-observer-hooks.md](docs/codex-observer-hooks.md) 与包内 [hooks/INSTALL.md](hooks/INSTALL.md)。

使用期间需要保持 Spellcast 运行。本地 MCP 地址为 `http://127.0.0.1:47194/mcp`。**接入界面当前仅支持 Codex**，Cursor、Claude Code、Windsurf 和“其他”保留显示，但置灰且不可选。
...
`npm start` 打开画布的浏览器预览；真实桌面气泡需要运行 `npm run desktop` 或已安装的应用。`scripts/` 中部分浏览器验收脚本依赖 Codex 内置的 Playwright 环境；隔离测试结果不代表真实 Codex 模型回合已通过。
