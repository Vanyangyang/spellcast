...
Spellcast 是给 coding agent 用的本地桌面应用。Agent 继续干活。有值得说的才会往你正在用的显示器上扔一条旁念，多半是吐槽或提醒，偶尔是个你没想到的想法。气泡叠在你正在做的事上面，不会把 Agent 窗口拽到前面。
...
我只在 Codex + Windows 上测过，这份预览也只保证这条路径。macOS 没测。

旁念和气泡是被动的。没有「生成气泡」按钮，我是故意不放的。打开「显示旁念」之后，只会通过宿主的子代理抛旁念，只占主任务一小块。有值得说的才出桌面气泡。Hooks 不保证每一轮都冒泡。
...
2. **画布上的结果**
   ...
   「待整理」里的一条 Idea。Agent 觉得值得留下才放上来。画布还在改，哪里别扭直接提 issue。现在就能用几种形式把想法摊开。后面可能加一个画布专注模式，不计成本，把创意工作推快一点。
...
- 已经发出去的包是 [0.4.9](https://github.com/Vanyangyang/spellcast/releases/tag/v0.4.9)，Windows x64 安装包。说明在 [0.4.9](docs/releases/0.4.9.md)。
...
1. **跟着你当前的屏幕走。** 内容来自 Agent 正在做的任务，显示位置跟前台窗口。气泡本来就少，多半是吐槽或提醒，偶尔是一条岔路。可以不理或拖走，要看再打开。
2. **收进来。** 点星标，把文字放进画布。知道原任务的话会带上。
3. **接着做。** 把组件收成一条有标题、有意图的 Idea。画布就是回复面，版式跟着内容走。
4. **改完再发。** 选方向或加约束，再改组件。这些都先留在画布上。点「发送到 Codex」才回原任务。要改发别的任务，需要确认。
...
一个 Idea 可以混用这些形式。切到别的应用后，桌面气泡会接着投。你自己把旁念暂停的话，会一直停着。
...
完成提醒是一张置顶小卡片。语音播报可选，语言跟界面走。Codex 卡片双击跳回原任务。Grok Build 卡片双击关掉。

独立旁念听 Spellcast 开关。由宿主子代理抛出，只占主任务一小块。细节在 [Codex Hooks 与验证边界](docs/codex-observer-hooks.md)。
...
2. 用 Codex 的话，打开「设置」，保持选中 Codex，安装或更新 Spellcast 接入。一次写入 MCP、Hooks 和行为 Skill，并做备份和冲突检查。然后重载 Codex，按接入状态里的提示信任 Hooks。
...
Spellcast 是跟 GPT-6 Astra 一起做的。Astra 会顶产品假设。Rust 核心和 Tauri 外壳是它写的，气泡到画布再回传也是它接上的。桌面交互和演示靠 computer use 跑通。关系图是 [AntV X6](https://github.com/antvis/X6)，桌面外壳是 [Tauri](https://github.com/tauri-apps/tauri)。
