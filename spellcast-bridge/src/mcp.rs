...
impl SpellcastMcp {
    ...
    #[tool(
        ...
        description = "Read aside switch state: enabled, paused, allowed, reason, policy_revision, and the App-selected locale. The App aside switch is the authority. Call this only when that state is missing or stale. unknown is not OFF; a prior OFF can be refreshed when fresh context next appears. This read does not authorize spawn or require a bubble. Submit checkpoints when enabled and not paused; only checkpoint status=ready starts a child. There is no MCP setter for the product switch."
    )]
    ...
    #[tool(
        ...
        description = "An isolated no-history observer submits its single decision directly. thought=null is valid silence; do not invent an aside. A thought must be grounded in the supplied snapshot, not progress or the main answer, and every user-visible thought field must use the brief's App-selected locale even when source material uses another language. A short line that leaves the conversation must still be readable; if tease is not enough, use body for who it applies to and why. Do not call spellcast_bubble to bypass the ticket. The ticket enforces original source, freshness and one completion; return only the compact status to the parent, never the thought."
    )]
