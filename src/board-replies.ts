...
const MESSAGES: Record<Locale, Record<string, string>> = {
  "zh-CN": {
    "nav.label": "板上回复",
    "nav.empty": "板上还没有回复。气泡被采纳后，Agent 的回复会在这里展开。",
    "nav.count": "{n} 条回复",
    "nav.new": "新",
    "nav.updated": "已更新",
  ...
  en: {
    ...
    "nav.empty": "No replies on the board yet. Once a bubble is adopted, the Agent's reply unfolds here.",
    "nav.count": "{n} replies",
    "nav.new": "New",
    "nav.updated": "Updated",
    ...
    "ask.done": "Sent. The reply will update on the board.",
  ...
  ja: {
    "nav.label": "ボード上の返信",
    "nav.empty": "まだ返信はありません。バブルが採用されると、Agent の返信がここに展開されます。",
    "nav.count": "{n} 件の返信",
    "nav.new": "新着",
    "nav.updated": "更新",
    ...
    "ask.done": "送信しました。返信はボード上で更新されます。",
...
class ReplyBoard {
  ...
  private setActive(replyId: string | null, restoreSelection: boolean): void {
    ...
    if (!restoreSelection) {
      if (this.selection && this.selection.reply_id !== replyId) this.setSelection(null);
      return;
    }
  ...
  private renderNav(): void {
    ...
    this.navCount.textContent = translate("nav.count", { n: this.replies.length });
  ...
  private createView(block: ReplyBlock): AnyBlockView {
    switch (block.type) {
      ...
      case "artifact":
        throw new Error("Open Web works use the unified Canvas renderer.");
    }
  }
